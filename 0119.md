...
* 흔히 사용하는 식별자; Uniform Resource **Locator**. 오늘날 대부분의 URI는 URL.
...
[PUT, POST의 차이는 멱등성과 리소스 결정권에 의해...](http://blog.embian.com/66)
...
HTTP는 응용 계층의 프로토콜이며, 네트워크 통신의 세부적인 것은 전송 제어 프로토콜; Transmission Control Protocol과 인터넷 프로토콜; Internet Protocol을 따른다. 둘을 합쳐서 흔히 인터넷 프로토콜 스위트라고 부르며 전송과 네트워크 계층에 속한다.
...
* host name은 DNS; Domain Name Service를 통해 IP주소로 변환된다
* port의 기본값은 80이다
...
예를 들어서 FTP 서버에 접근하고싶은 HTTP 요청이 있을 때, 게이트웨이는 요청을 FTP로, 응답을 HTTP로 변환한다.
...
* 전송계층보안을 위해 사용되는 터널의 예로 TLS, SSL이 있다
...
* 표현 방식에 따라 URL, URN의 하위 개념으로 분류할 수 있다.
* URL은 고정된 위치를 나타내며, URN은 위치에 구애받지 않는 식별자
* Scheme + Domain + Port + Path + Query + Fragment
* 사람이 알기 쉬운 Domain은 DNS에 의해서 IP주소로 변경된다
...
* 메세지는 Starting line, Header(s), Body 세 부분으로 구성
    ...
    * Header(s): 요청에 필요한 메타데이터(이름과 값으로 이루어진)들을 담고있으며 반드시 empty line으로 끝나야한다
    * Body: 일부 메소드는 요청/응답에 데이터가 수반된다. 해당 데이터는 body에 담긴다.

HTTP 트랜잭션 환경

* 전송 제어 규약(TCP)와 인터넷 규약(IP)를 합쳐서 인터넷 프로토콜 스위트 또는 TCP/IP라고 칭한다
* HTTP 통신은 주로 TCP/IP 환경 위에서 이루어진다.
* OSI 7 Layer에서 TCP는 전송 계층, IP는 네트워크 계층에 해당한다
